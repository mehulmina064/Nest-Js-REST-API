...
let TestService = class TestService {
    constructor(processTestRepository, zohoTokenRepository) {
        this.processTestRepository = processTestRepository;
        this.zohoTokenRepository = zohoTokenRepository;
    }
    findOne(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let team = yield this.processTestRepository.findOne(id);
            if (team) {
                return team;
            }
            else {
                return Promise.reject(new common_2.HttpException('Test not found', common_2.HttpStatus.BAD_REQUEST));
            }
        });
    }
    save(role) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let check = yield this.processTestRepository.findOne({ where: { name: role.name } });
            if (check) {
                return Promise.reject(new common_2.HttpException('Test already exists', common_2.HttpStatus.BAD_REQUEST));
            }
            else {
                role.status = test_entity_1.Status.ACTIVE;
                role.fields = [];
                role.testValues = [];
                return yield this.processTestRepository.save(role);
            }
        });
    }
    softRemove(id, userId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.processTestRepository.findOne(id);
            if (role) {
                role.deletedAt = new Date();
                role.deletedBy = userId;
                role.status = test_entity_1.Status.DELETED;
                return yield this.processTestRepository.save(role);
            }
            else {
                return Promise.reject(new common_2.HttpException('Test not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    hardRemove(id, userId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.processTestRepository.findOne(id);
            if (role) {
                let del = yield this.processTestRepository.delete(id);
                if (del) {
                    return "Deleted successfully";
                }
                else {
                    return Promise.reject(new common_2.HttpException('Test not Deleted', common_2.HttpStatus.INTERNAL_SERVER_ERROR));
                }
            }
            else {
                return Promise.reject(new common_2.HttpException('Test not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    update(id, updateRole) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.processTestRepository.findOne(id);
            if (role) {
                let data = yield this.processTestRepository.update(id, updateRole);
                let saveRole = yield this.processTestRepository.findOne(id);
                return saveRole;
            }
            else {
                return Promise.reject(new common_2.HttpException('Test not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    findAll(query) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (query) {
                console.log(query);
                let data = yield this.processTestRepository.findAndCount(query);
                return { data: data[0], count: data[1] };
            }
            else {
                let data = yield this.processTestRepository.findAndCount();
                return { data: data[0], count: data[1] };
            }
        });
    }
    check(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let check = yield this.processTestRepository.findOne(id).then((res1) => {
                return res1;
            }).catch((err) => {
                return false;
            });
    ...
    addFields(id, userId, fields) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let PG = yield this.processTestRepository.findOne(id);
            if (PG) {
                ...
                yield this.processTestRepository.update(PG.id, PG);
                return PG;
            }
            else {
                return Promise.reject(new common_2.HttpException('Test not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    addTestValues(id, userId, testValues) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let PG = yield this.processTestRepository.findOne(id);
            if (PG) {
    ...
    editTestValues(id, userId, testValues) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let PG = yield this.processTestRepository.findOne(id);
            if (PG) {
                for (let i = 0; i < testValues.length; i++) {
                ...
                PG.testValues = yield PG.testValues.map(obj => testValues.find(o => o.name === obj.name) || obj);
                PG.updatedBy = userId;
                PG.updatedAt = new Date();
                return PG;
            }
...
TestService = tslib_1.__decorate([
    common_1.Injectable(),
    tslib_1.__param(0, typeorm_1.InjectRepository(test_entity_1.Test)),
