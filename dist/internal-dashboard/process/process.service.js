...
let processService = class processService {
    constructor(processRepository, zohoTokenRepository) {
        this.processRepository = processRepository;
        this.zohoTokenRepository = zohoTokenRepository;
    }
    findOne(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let team = yield this.processRepository.findOne(id);
            if (team) {
                return team;
            }
            else {
                return Promise.reject(new common_2.HttpException('process not found', common_2.HttpStatus.BAD_REQUEST));
            }
        });
    }
    save(role) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let check = yield this.processRepository.findOne({ where: { name: role.name } });
            if (check) {
                return Promise.reject(new common_2.HttpException('process already exists', common_2.HttpStatus.BAD_REQUEST));
            }
            else {
                role.status = process_entity_1.Status.ACTIVE;
                role.fields = [];
                return yield this.processRepository.save(role);
            }
        });
    }
    softRemove(id, userId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.processRepository.findOne(id);
            if (role) {
                role.deletedAt = new Date();
                role.deletedBy = userId;
                role.status = process_entity_1.Status.DELETED;
                return yield this.processRepository.save(role);
            }
            else {
                return Promise.reject(new common_2.HttpException('process not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    hardRemove(id, userId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.processRepository.findOne(id);
            if (role) {
                let del = yield this.processRepository.delete(id);
                if (del) {
                    return "Deleted successfully";
                }
                else {
                    return Promise.reject(new common_2.HttpException('process not Deleted', common_2.HttpStatus.INTERNAL_SERVER_ERROR));
                }
            }
            else {
                return Promise.reject(new common_2.HttpException('process not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    update(id, updateRole) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.processRepository.findOne(id);
            if (role) {
                let data = yield this.processRepository.update(id, updateRole);
                let saveRole = yield this.processRepository.findOne(id);
                return saveRole;
            }
            else {
                return Promise.reject(new common_2.HttpException('process not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    findAll(query) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (query) {
                console.log(query);
                let data = yield this.processRepository.findAndCount(query);
                return { data: data[0], count: data[1] };
            }
            else {
                let data = yield this.processRepository.findAndCount();
                return { data: data[0], count: data[1] };
            }
        });
    }
    check(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let check = yield this.processRepository.findOne(id).then((res1) => {
                return res1;
            }).catch((err) => {
                return false;
            });
    ...
    addFields(id, userId, fields) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let PG = yield this.processRepository.findOne(id);
            if (PG) {
                ...
                yield this.processRepository.update(PG.id, PG);
                return PG;
            }
            else {
                return Promise.reject(new common_2.HttpException('process not found', common_2.HttpStatus.BAD_REQUEST));
...
processService = tslib_1.__decorate([
    common_1.Injectable(),
    tslib_1.__param(0, typeorm_1.InjectRepository(process_entity_1.process)),
