...
let parentSkuService = class parentSkuService {
    constructor(parentSkueRepository, zohoTokenRepository) {
        this.parentSkueRepository = parentSkueRepository;
        this.zohoTokenRepository = zohoTokenRepository;
    }
    findOne(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let team = yield this.parentSkueRepository.findOne(id);
            if (team) {
                return team;
            }
            else {
                return Promise.reject(new common_2.HttpException('parentSku not found', common_2.HttpStatus.BAD_REQUEST));
            }
        });
    }
    save(role) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let check = yield this.parentSkueRepository.findOne({ where: { name: role.name } });
            if (check) {
                return Promise.reject(new common_2.HttpException('parentSku already exists', common_2.HttpStatus.BAD_REQUEST));
            }
            else {
                role.status = parentSku_entity_1.Status.ACTIVE;
                role.fields = [];
                return yield this.parentSkueRepository.save(role);
            }
        });
    }
    softRemove(id, userId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.parentSkueRepository.findOne(id);
            if (role) {
                role.deletedAt = new Date();
                role.deletedBy = userId;
                role.status = parentSku_entity_1.Status.DELETED;
                return yield this.parentSkueRepository.save(role);
            }
            else {
                return Promise.reject(new common_2.HttpException('parentSku not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    hardRemove(id, userId) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.parentSkueRepository.findOne(id);
            if (role) {
                let del = yield this.parentSkueRepository.delete(id);
                if (del) {
                    return "Deleted successfully";
                }
                else {
                    return Promise.reject(new common_2.HttpException('parentSku not Deleted', common_2.HttpStatus.INTERNAL_SERVER_ERROR));
                }
            }
            else {
                return Promise.reject(new common_2.HttpException('parentSku not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    update(id, updateRole) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let role = yield this.parentSkueRepository.findOne(id);
            if (role) {
                let data = yield this.parentSkueRepository.update(id, updateRole);
                let saveRole = yield this.parentSkueRepository.findOne(id);
                return saveRole;
            }
            else {
                return Promise.reject(new common_2.HttpException('parentSku not found', common_2.HttpStatus.BAD_REQUEST));
    ...
    findAll(query) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (query) {
                console.log(query);
                let data = yield this.parentSkueRepository.findAndCount(query);
                return { data: data[0], count: data[1] };
            }
            else {
                let data = yield this.parentSkueRepository.findAndCount();
                return { data: data[0], count: data[1] };
            }
        });
    }
    check(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let check = yield this.parentSkueRepository.findOne(id).then((res1) => {
                return res1;
            }).catch((err) => {
                return false;
            });
    ...
    addFields(id, userId, fields) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let PG = yield this.parentSkueRepository.findOne(id);
            if (PG) {
                ...
                yield this.parentSkueRepository.update(PG.id, PG);
                return PG;
            }
            else {
                return Promise.reject(new common_2.HttpException('parentSku not found', common_2.HttpStatus.BAD_REQUEST));
...
parentSkuService = tslib_1.__decorate([
    common_1.Injectable(),
    tslib_1.__param(0, typeorm_1.InjectRepository(parentSku_entity_1.parentSku)),
    tslib_1.__param(1, typeorm_1.InjectRepository(token_entity_1.zohoToken)),
    tslib_1.__metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository])
], parentSkuService);
exports.parentSkuService = parentSkuService;
//# sourceMappingURL=parentSku.service.js.map