...
@Controller('zohoPurchaseOrder')
export class zohoPurchaseOrderController {
  constructor(  @InjectRepository(User)
  private readonly userRepository: Repository<User>,
  private readonly zohoPurchaseOrderService:zohoPurchaseOrderService,
  ) { }
  ...
  @Query('page', new DefaultValuePipe(1), ParseIntPipe) page: number = 1,
    ...
    if (user.roles.includes(UserRole.PRODO_ADMIN)){
      ...
      let attrFilter1=[]
        ...
        result = await this.zohoPurchaseOrderService.findAll(query1) 
        }
      else{
        result = await this.zohoPurchaseOrderService.findAll(query1)  
      }
    ...
    return {statusCode:200,message:"All Orders",count:result.count,limit:limit,page:page,data:result.data}
      ...
      // return query
        result = await this.zohoPurchaseOrderService.findAll(query)  
    result.data = result.data.slice(start, end) 
        return {statusCode:200,message:"All Orders",count:result.count,limit:limit,page:page,data:result.data}
    }
  }
...
@Post('syncOneOrder')
async updateSalesOrder(@Body('purchaseorder_id') id:any) { 
  // var id=purchaseorder.purchaseorder_id
  if(id){
    let result = await this.zohoPurchaseOrderService.saveFromZohoId(id)
...
async updateSalesOrders(@Param('start') start: any){
  ...
  for(page;page>0;page++){
    let data=await this.zohoPurchaseOrderService.zohoAllPo(page)
    if(data.count){ 
      let Orders=data.data
    // for(let i=start;i<Orders.length;i++){
    for(let i=start;i<100;i++){
        let order=Orders[i]
        let out = await this.zohoPurchaseOrderService.saveFromZohoId(order.purchaseorder_id)
...
async zohoOnePo(@Param('id') id: any){
  let data= await this.zohoPurchaseOrderService.InventoryPorByID(id)
  return {statusCode:200,message:"Order Details from zoho mapped",data:data}
}

@Get('getAttachment/:purchaseOrderId')
@Header('Content-Type', 'application/pdf')
@Header('Content-Disposition', 'attachment; filename=Attachment.pdf')
async getAttachment(@Param('purchaseOrderId') purchaseOrderId: any,@Response() res: any): Promise<Buffer>{
    throw new HttpException({
        status: HttpStatus.NOT_IMPLEMENTED,
        error: 'NOT_IMPLEMENTED',
        message: "NOT_IMPLEMENTED",
      }, HttpStatus.NOT_IMPLEMENTED);
  var attachment=await this.zohoPurchaseOrderService.getAttachment(purchaseOrderId)
  if(attachment){
    attachment.pipe(res)
    return res
  }
...
@Get('OrderSummary/:purchaseOrderId')
@Header('Content-Type', 'application/pdf')
@Header('Content-Disposition', 'attachment; filename=OrderSummary.pdf')
async OrderSummary(@Param('purchaseOrderId') purchaseOrderId: any,@Response() res: any): Promise<Buffer>{
  var OrderSummary=await this.zohoPurchaseOrderService.OrderSummary(purchaseOrderId)
  if(OrderSummary){
