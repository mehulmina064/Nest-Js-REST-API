...
export class processController {
  ...
  @Query('isDefault',new DefaultValuePipe('NA')) isDefault: string="NA",
    ...
    if (user.roles.includes(UserRole.PRODO_ADMIN)){
      // console.log("error")
      // return user
      result = await this.processService.findAll(query)    
  ...
  async findOne(@Param('id') id: string) {
    let testProcess = await this.testProcessService.findAll({where: { processId: id}})    
    ...
    if(testProcess.count){
      ...
      let yep = await this.TestService.findAll({where : { $or : [...batchItemProcessIds]}})
      // return yep.data
  ...
  async save(@Body() role: CreateProcessDto,@Request() req:any) {
      role.updatedBy=req.user.id;
      role.createdBy=req.user.id;
      let save= await this.processService.save(role);
      if(save){
  ...
  @UseGuards(IJwtAuthGuard)
  @UsePipes(new ValidationPipe({ transform: true }))
  async update(@Param('id') id: string, @Body() role: UpdateProcessDto,@Request() req:any) {
   // return role
   if(role.status==Status.DELETED){
     throw new HttpException({
      ...
      let save= await this.processService.update(id, role);
      if(save){
  ...
  async softDelete(@Param('id') id: string,@Request() req:any) {
      let save=await this.processService.softRemove(id,req.user.id);
      if(save){
  ...
  async hardDelete(@Param('id') id: string,@Request() req:any) {
      ...
      {
       let save=await this.processService.hardRemove(id,req.user.id);
       if(save){
...
body: CreateFieldDto[],) {
  ...
  if(new Set(d).size !== d.length)
  {
    ...
    let save= await this.processService.addFields(id,req.user.id,body);
    if(save){
...
body: CreateFieldDto[],) {
  ...
  if(new Set(d).size !== d.length)
  {
    ...
    let save= await this.processService.editFields(id,req.user.id,body);
    if(save){
