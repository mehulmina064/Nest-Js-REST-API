...
export class logisticsController {
  ...
  @Query('page', new DefaultValuePipe(1), ParseIntPipe) page: number = 1,
    ...
    if (user.roles.includes(UserRole.PRODO_ADMIN)){
      // console.log("error")
      // return user
      result = await this.logisticService.findAll(query)    
  ...
  async save(@Body() role: CreateLogisticDto,@Request() req:any) {
      role.updatedBy=req.user.id;
      role.createdBy=req.user.id;
      let save= await this.logisticService.save(role);
      if(save){
  ...
  @UseGuards(IJwtAuthGuard)
  @UsePipes(new ValidationPipe({ transform: true }))
  async update(@Param('id') id: string, @Body() role: UpdateLogisticDto,@Request() req:any) {
   // return role
   if(role.status==Status.DELETED){
     throw new HttpException({
      ...
      let save= await this.logisticService.update(id, role);
      if(save){
  ...
  async softDelete(@Param('id') id: string,@Request() req:any) {
      let save=await this.logisticService.softRemove(id,req.user.id);
      if(save){
  ...
  async hardDelete(@Param('id') id: string,@Request() req:any) {
      ...
      {
       let save=await this.logisticService.hardRemove(id,req.user.id);
       if(save){
