...
async findOne(id: string){
  let team = await this.batchRepository.findOne(id);  
  if(team){
  return team
 }
 else{
...
async save(role: batch){
// let check=await this.batchRepository.findOne({where:{name:role.name}});
// if(check){
//   return Promise.reject(new HttpException('Batch already exists', HttpStatus.BAD_REQUEST));
// }
// else{
  ...
  role.fields=[]
  // return role
  return await this.batchRepository.save(role)
// }
}
...
async softRemove(id: string,userId:string) {
  let role = await this.batchRepository.findOne(id);  
  if(role){
  let del= await this.batchRepository.delete(id);
  if(del){
    return "Deleted successfully"
  }
  else{
    return Promise.reject(new HttpException('Batch not Deleted', HttpStatus.INTERNAL_SERVER_ERROR));
  }
 }
 else{
      return Promise.reject(new HttpException('Batch not found', HttpStatus.BAD_REQUEST));
...
async update(id: string,updateRole: Partial<batch>): Promise<batch> {
  let role = await this.batchRepository.findOne(id);  
  if(role){
    let data= await this.batchRepository.update(id, updateRole);
    let saveRole=await this.batchRepository.findOne(id);
    return saveRole;
 }
 else{
      return Promise.reject(new HttpException('Batch not found', HttpStatus.BAD_REQUEST));
...
// console.log("in findall")
  if(query){
    console.log(query)
    let data=await this.batchRepository.findAndCount(query)
    return {data:data[0],count:data[1]}
  } 
  else {
    let data=await this.batchRepository.findAndCount()
    return {data:data[0],count:data[1]}
  }
}

async check(id: string){
  let check = await this.batchRepository.findOne(id).then((res1)=>{
    return res1
  }).catch((err)=>{
    return false
  })
...
async addFields(id:string,userId:string,fields:any){
  let PG = await this.batchRepository.findOne(id);
  if(PG){//permission group
    ...
    await this.batchRepository.update(PG.id,PG)
    return PG  
   }
   else{
        return Promise.reject(new HttpException('Batch not found', HttpStatus.BAD_REQUEST));
